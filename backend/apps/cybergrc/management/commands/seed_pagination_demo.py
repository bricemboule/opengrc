...
class Command(BaseCommand):
    ...
    def ensure_consultations(self, organization, today):
        def build_consultation_defaults(index, title):
            schedule_date = today + timedelta(days=index * 5)
            start_datetime = timezone.make_aware(datetime.combine(schedule_date, time(hour=9 + (index % 5), minute=0)))
            end_datetime = start_datetime + timedelta(hours=1, minutes=30)
            engagement_channel = [
                EngagementChannel.IN_PERSON,
                EngagementChannel.VIDEO,
                EngagementChannel.HYBRID,
            ...
            return {
                ...
                "meeting_link": f"https://meet.opengrc.local/consultation-{index:02d}" if engagement_channel in {EngagementChannel.VIDEO, EngagementChannel.HYBRID} else "",
                "dial_in_details": f"Dial +220 300 10{index:02d} | PIN {4100 + index}" if engagement_channel in {EngagementChannel.PHONE, EngagementChannel.HYBRID} else "",
                "meeting_location": f"Coordination Room {((index - 1) % 4) + 1}" if engagement_channel in {EngagementChannel.IN_PERSON, EngagementChannel.HYBRID} else "",
                "start_datetime": start_datetime,
                "end_datetime": end_datetime,
                "objective": f"Consultation objective {index:02d}.",
                "agenda": f"Agenda item 1 for consultation {index:02d}.\nAgenda item 2 for consultation {index:02d}.",
                "attendees": f"Lead {index:02d}\nstakeholder{index:02d}@example.org\ncoordination@opengrc.local",
                "status": status,
                "focal_person": f"Consultation Lead {index:02d}",
                "minutes": "Generated by seed_pagination_demo." if status == ConsultationStatus.COMPLETED else "",
                "outcome_summary": "Generated by seed_pagination_demo." if status in {ConsultationStatus.COMPLETED, ConsultationStatus.MISSED} else "",
                "follow_up_actions": f"Follow-up actions {index:02d}.",
                "next_follow_up_date": schedule_date + timedelta(days=7),
                "notes": "Generated by seed_pagination_demo.",
            }
